...
class SearchController extends AppController{
	...
	private function _showIndex() {
		...
		//-----------------------------------------------------------------------------------------	
		//都道府県リスト
		$PREFECTURES_types = Configure::read('PREFECTURES_MAP');
		...
		//-----------------------------------------------------------------------------------------
		//car_types
		//$this->set('PREFECTURES_types', $PREFECTURES_types);
		$this->set('PREFECTURES_types', $PREFECTURES_types);
	...
	public function searchDetail() {
		...
		$municipality = $result_detail[0]['ModelData']['municipality'];
		//$car_name	= $result_detail[0]['ModelData']['car_name'];
		//$model	  	= $result_detail[0]['ModelData']['model'];
		$prefectures	= $result_detail[0]['ModelData']['prefectures'];
	...
	public function getMakerlist() {
		...
		//バリデーション
		if(!preg_match(PREG_CAR_TYPE, $form_info['prefectures'])) {
			return json_encode($return_json);
		}
		...
		//-----------------------------------------------------------------------------------------
		$municipality = array();
		$fields = array('DISTINCT municipality');
		$order 	= array('municipality asc');
		$conditions = array('car_type'=> $form_info['prefectures']); 
		$result = true;
		$result = $this->_selectModelData($fields, $order, $conditions, $municipality);
		if(!$result) {
			//DB取得失敗エラー
			return false;
			$this->showErrorPage_500();
			...
			for($i = 0; $i < sizeof($municipality); $i++) {
				$return_json[$i+1] =  array('view'  => $municipality[$i]['ModelData']['maker_name']);
				$return_json[$i+1] += array('value' => $municipality[$i]['ModelData']['maker_name']);
			}
		}
		return json_encode($return_json);
	}
